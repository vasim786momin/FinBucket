...
public class CalculateEmiMortageFinal extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_calculate_emi_mortage);
        ...
        System.out.println("Values for fetching data on Mortage final " + name + "\t" + gender + "\t" + dob);
        System.out.println("Values for fetching data" + city + "\t" + email + "\t" + mobile);
                    ...
                    if(propType.equals("PropertyTypes"))
                    {
                        ...
                        else
                        {
                            ...
                            try{
                                ...
                                if( minc<10000)
                                {
                                    Toast.makeText(getApplicationContext(),"Minimum Monthly salary 10000",Toast.LENGTH_SHORT).show();
                                }
                                else
                                {
                                    if( lamt <1000000 || lamt > 5000000)
                                    {
                                        Toast.makeText(getApplicationContext(),"Loan Amount between 10 lakh - 50 lakh",Toast.LENGTH_SHORT).show();
                                    }
                                    else
                                    {
                                        if( ltime > 30)
                                        ...
                                        else
                                        {
                                            if( lemi <10000)
                                            {
                                                Toast.makeText(getApplicationContext(),"Minimum Emi 10000",Toast.LENGTH_SHORT).show();
                                            }
                                            else
                                            {
                                                ...
                                                String mortageInsertUrl ="http://www.finbucket.com/index.php/android/mortgageloan?name="+name+"&number="+mobile+"&city="+city+"&loanammount="+loanAmount+"&income="+mIncome+"&dob="+dob+"& email="+ email+"& gender="+gender+"&type_of_property="+propType+"& emi="+ suitableEmi+"&time="+loanTenure+"& emptype=" +empType;

                                                System.out.println(mortageInsertUrl);



                                                mortageInsertUrl = mortageInsertUrl.replaceAll(" ", "%20");

                                                System.out.println("After replace :"+mortageInsertUrl);

                                                MortgageLoanInsertAsync obj = new MortgageLoanInsertAsync();
                                                obj.execute(mortageInsertUrl);
                                            }
                                        }
                                    }
                                }
    ...
    private class MortgageLoanInsertAsync extends AsyncTask<String, Void, String>
    {
        ...
        {
            super.onPreExecute();
            pd = new ProgressDialog(CalculateEmiMortageFinal.this);
        ...
        protected void onPostExecute(String result)
        {
            ...
            {
                Intent i = new Intent(CalculateEmiMortageFinal.this,MortageCalculatedEmiList.class);
